...
function CombatStatus({ network }: { network: ArenaNetwork }) {
 ...
 const self = snapshot.players.find(p => p.playerProfileId === network.playerId)!;
 return <section className="combat-status" aria-label="Combat status"><p className={snapshot.match.remainingMs<=10000?"match-clock final-seconds":"match-clock"} aria-label="Match time remaining">Time: {Math.floor(Math.ceil(snapshot.match.remainingMs/1000)/60)}:{String(Math.ceil(snapshot.match.remainingMs/1000)%60).padStart(2,"0")}</p><div><span>Basic Blaster</span><strong>Health: {self.health} / {self.maxHealth}</strong></div><p role="status">{self.alive ? "Alive" : `Respawning in ${(self.respawnInMs / 1000).toFixed(1)}s`}</p><p>Score: {self.score} · Eliminations: {self.eliminations} <span>·</span> Deaths: {self.deaths}</p></section>;
}
export function ArenaHUD({ network, name, count, connection, leave, busy, error }: { network: ArenaNetwork; name: string; count: number; connection: string; leave: () => void; busy: boolean; error: string }) {
 ...
 return <main id="main" className="player-page arena-page"><header className="arena-hud"><div><p className="eyebrow">LIVE ARENA</p><h1>{name}</h1><p role="status">{count} players · {connection}</p><p className="field-help">Match {network.latest.gameId.slice(0,8)}</p></div><button className="button button-secondary" onClick={leave} disabled={busy}>Leave Arena</button></header>{error && <p role="alert">{error}</p>}<CombatStatus network={network} /><GameLoader network={network} /><div className="arena-controls"><p>Move: WASD / Arrows <span>· Aim: Mouse · Fire: Left Click (hold)</span></p><button className="button button-secondary" aria-pressed={debug} onClick={() => setDebug(!debug)}>Network debug</button></div>{debug && <Diagnostics network={network} />}</main>;
}
