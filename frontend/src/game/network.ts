...
export class ArenaNetwork {
 ...
 accept(snapshot: GameSnapshot) {
  ...
  if (!self) return;
  if(snapshot.match.status==="FINISHED")this.pending=[];
 ...
 combat(fire: boolean, x: number, y: number) {
  ...
  if (this.latest.match.status!=="IN_GAME" || !self?.alive || !self.connected || !this.socket.connected || performance.now() - this.receivedAt > 1000 || length < 0.001) return;
 ...
 input(intent: Pick<GameInput, "up" | "down" | "left" | "right">) {
  if (this.latest.match.status!=="IN_GAME" || !this.latest.players.find(p => p.playerProfileId === this.playerId)?.alive || !this.latest.players.find(p => p.playerProfileId === this.playerId)?.connected || !this.socket.connected || performance.now() - this.receivedAt > 1000 || this.pending.length >= 10) return;
