...
try{
 ...
 await until(()=>latest.every(s=>s.match.status==='FINISHED'),35000);
 const result=latest[0].match.result;assert.deepEqual(result,latest[1].match.result);assert.equal(result.winnerPlayerProfileId,ids[0]);assert.equal(latest[0].match.remainingMs,0);assert.equal(latest[0].projectiles.length,0);
 ...
 await until(()=>latest.every(s=>s.match.persistence==='SAVED'));
 ...
 for(const p of pages)await p.getByRole('button',{name:"I'm ready",exact:true}).waitFor();
 await b.getByRole('button',{name:"I'm ready",exact:true}).click();await a.getByRole('button',{name:"I'm ready",exact:true}).click();await a.getByRole('button',{name:'Start Match',exact:true}).click();
 for(const p of pages)await p.locator('canvas').waitFor();await until(()=>latest.every(s=>s.gameId!==gameId&&s.match.status==='IN_GAME'));
 assert.equal(latest[0].gameId,latest[1].gameId);assert.ok(latest[0].players.every(p=>p.score===0&&p.eliminations===0&&p.deaths===0&&p.health===100));
 const rematchId=latest[0].gameId;
 await until(()=>latest.every(s=>s.match.status==='FINISHED'&&s.match.persistence==='SAVED'),35000);assert.equal(latest[0].match.result.tied,true);
 ...
 console.log(JSON.stringify({result:'PASS',scenario:'Two players, timer, elimination/score, finish/winner, identical saved standings, finished refresh, return/reset, fresh rematch, tied result, authenticated history, exact database rows, tablet/mobile results/history, no console errors',snapshotCounts:history.map(h=>h.length),errors,consoleErrors,artifacts}));
}catch(error){
