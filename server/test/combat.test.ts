...
test('four hits eliminate once and update live attacker/victim counters',()=>{const {game}=setup();kill(game);const victim=game.players.get('b')!.state;assert.equal(victim.health,0);assert.equal(victim.deaths,1);assert.equal(game.players.get('a')!.state.eliminations,1);assert.equal(game.players.get('a')!.state.score,1);fire(game);step(game,10);assert.equal(victim.health,0);assert.equal(victim.deaths,1);});
